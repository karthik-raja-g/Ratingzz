...
<body>
    <%  Map<Integer,Map<Integer,Integer>> stats = (Map<Integer,Map<Integer,Integer>>) request.getAttribute("formStats") ;
        Set<Question> questions = (Set<Question>)request.getAttribute("questions") ;
        for(Question question : questions) {%>
        <form id="form1" onclick="drawChart(); return false" >
            <label for="1star">1 Star: </label><input type="number" value ="<%=stats.get(question.getId()).get(1)%>" id="1star"><br>
            <label for="2star">2 Star: </label><input type="number"  value ="<%=stats.get(question.getId()).get(2)%>" id="2star"><br>
            <label for="3star">3 Star: </label><input type="number" value ="<%=stats.get(question.getId()).get(3)%>" id="3star"><br>
            <label for="4star">4 Star: </label><input type="number" value ="<%=stats.get(question.getId()).get(4)%>" id="4star"><br>
            <label for="5star">5 star: </label><input type="number" value ="<%=stats.get(question.getId()).get(5)%>" id="5star"><br>
